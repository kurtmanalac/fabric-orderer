...
enroll_json=$(jq -n --arg id "$ENROLL_ID" --arg pw "$ENROLL_PW" --arg id "orderer" --arg cmd "$customCmd" '{userId: $id, userPw: $pw, idType: $id, customCmd: $cmd}')
...
CA_CERT_FILE=${CA_CERT_FILE:-$FABRIC_CA_CLIENT_HOME/$ENROLL_ID/msp/cacerts/ca-cert.pem}
