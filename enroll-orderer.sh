...
customCmd=${customCmd:---csr.hosts $RAILWAY_SERVICE_NAME,$RAILWAY_PRIVATE_DOMAIN --csr.names C=US,ST=California,L=SanFrancisco,O=upmo,OU=orderer}
enroll_json=$(jq -n --arg id "$ENROLL_ID" --arg pw "$ENROLL_PW" --arg type "orderer" --arg cmd "$customCmd" '{userId: $id, userPw: $pw, idType: $type, customCmd: $cmd}')
...
echo "🔐 Enrolling orderer with Fabric CA..."
...
KEYFILE=$(find $FABRIC_CA_CLIENT_HOME/ord1/tls/keystore -type f -name '*_sk')
...
export ORDERER_GENERAL_TLS_CERTIFICATE=$FABRIC_CA_CLIENT_HOME/ord1/tls/signcerts/cert.pem
echo "check this other export: $ORDERER_GENERAL_TLS_CERTIFICATE"

ADMINKEYFILE=$(find $FABRIC_CA_CLIENT_HOME/ord1/tls/keystore -type f -name '*_sk')
export ORDERER_ADMIN_TLS_PRIVATEKEY=$ADMINKEYFILE
export ORDERER_ADMIN_TLS_CERTIFICATE=$FABRIC_CA_CLIENT_HOME/ord1/tls/signcerts/cert.pem
